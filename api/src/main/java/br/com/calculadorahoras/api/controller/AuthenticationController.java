...
public class AuthenticationController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginDTO loginDTO) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ErrorResponse("Este usuário não existe: " + loginDTO.username()));
        }
        catch (AuthenticationException e){
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(new ErrorResponse(e.getMessage()));
        }
        catch (Exception e) {
            return ResponseEntity.internalServerError().body(new ErrorResponse("Erro no processamento: " + e.getClass()));
        }
